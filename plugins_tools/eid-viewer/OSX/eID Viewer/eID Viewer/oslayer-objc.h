...
@protocol eIDOSLayerUI < NSObject > @optional - (void) newsrc:(eIDSource)
	which;
				///< called by eid_vwr_ui_callbacks::newsrc()

-(void) newstringdata: (NSString *) data /**< . */ withLabel:(NSString *)
	label;
				///< called by eid_vwr_ui_callbacks::newstringdata()
-(void) newbindata:(NSData *)
     data withLabel:(NSString *) label;	///< called by eid_vwr_ui_callbacks::newbindata()

-(void) log:(NSString *)
     line withLevel:(eIDLogLevel) level;	///< called by eid_vwr_ui_callbacks::log()

-(void) newstate:(eIDState) state;
				///< called by eid_vwr_ui_callbacks::newstate()
-(void) pinop_result:(eIDResult)
     result forOperation:(eIDPinOp) operation;	///< called by eid_vwr_ui_callbacks::pinop_result()

-(void) readersFound:(NSArray *)
     readers withSlotNumbers:(NSArray *) slots;	///< called by eid_vwr_ui_callbacks::readers_changed()

@end

/** Class method-only class which wraps the corresponding C-only APIs for the
 benefit of ObjC applications. */
@ interface eIDOSLayerBackend: NSObject + (void) pinop:(eIDPinOp)
	which;
				///< calls eid_vwr_pinop()
...
+(eIDResult) validateCert:(NSData *)
     certificate withCa:(NSData *) ca;	///< calls eid_vwr_verify_cert() with valid perform_ocsp_request and free_ocsp_request function pointers
