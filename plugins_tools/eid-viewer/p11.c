...
int eid_vwr_p11_do_pinop(void* data) {
	...
	if((retval = eid_vwr_p11_do_pinop_real((enum eid_vwr_pinops)p)) != CKR_OK) {
		be_pinresult((enum eid_vwr_pinops)p, EID_VWR_FAILED);
	} else {
		be_pinresult((enum eid_vwr_pinops)p, EID_VWR_SUCCESS);
	}
	return retval;
}
