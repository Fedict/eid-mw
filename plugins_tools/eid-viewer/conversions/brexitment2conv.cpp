#include "permstayconv.h"

std::map < eid_vwr_langs, std::map <char, EID_STRING> > PermStayConvertor::conversions;

PermStayConvertor::PermStayConvertor() {
...
EID_STRING PermStayConvertor::convert(const void *normal) {
	const char *norm = (const char*)normal;
	return conversions[target_][*norm];
}

std::map < EID_STRING, EID_STRING > XmlPermStayConvertor::conversions;

XmlPermStayConvertor::XmlPermStayConvertor() {
	if(conversions.size() == 0)
	{
...
EID_STRING XmlPermStayConvertor::convert(const void *normal) {
