...
"%SIGNTOOL_PATH%\signtool" sign /fd SHA256 /s MY /n "Zetes SA" /sha1 "06f01865ee31c88ef2bc9d6f4b3eff06427d1ea7" /tr http://rfc3161timestamp.globalsign.com/advanced /td SHA256 /v "%MDRVINSTALLPATH%\beidmdrv\beidmdrv32.dll"
@if "%ERRORLEVEL%" == "1" goto signtool_failed

@echo [INFO] Sign the minidriver 64bit dll
"%SIGNTOOL_PATH%\signtool" sign /fd SHA256 /s MY /n "Zetes SA" /sha1 "06f01865ee31c88ef2bc9d6f4b3eff06427d1ea7" /tr http://rfc3161timestamp.globalsign.com/advanced /td SHA256 /v "%MDRVINSTALLPATH%\beidmdrv\beidmdrv64.dll"
@if "%ERRORLEVEL%" == "1" goto signtool_failed

:: Sign catalog
:: ============
@echo [INFO] Sign the minidriver catalog
"%SIGNTOOL_PATH%\signtool" sign /as /fd SHA256 /ac "%MDRVCERTPATH%\MSCV-GlobalSign Root CA.cer" /s MY /n "Zetes SA" /sha1 "06f01865ee31c88ef2bc9d6f4b3eff06427d1ea7" /tr http://rfc3161timestamp.globalsign.com/advanced /td SHA256 /v "%MDRVINSTALLPATH%\beidmdrv\beidmdrv.cat"
@if "%ERRORLEVEL%" == "1" goto signtool_failed

@mkdir "%~dp0..\..\installers\quickinstaller\Drivers\XP-WIN8\beidmdrv\"
@if exist "%~dp0..\..\installers\quickinstaller\Drivers\XP-WIN8\beidmdrv\beidmdrv.cat" del "%~dp0..\..\installers\quickinstaller\Drivers\XP-WIN8\beidmdrv\beidmdrv.cat"
