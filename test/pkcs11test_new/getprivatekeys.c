...
testRet test_getprivatekeys() {
	...
	if (InitializeTest(&handle,&functions))
	{
		...
		if (ReturnedSuccesfull(frv,&(retVal.pkcs11rv), "C_Initialize", "test_getprivatekeys" ))
		{		
			frv = (*functions->C_GetSlotList) (0, 0, &slot_count);
			if (ReturnedSuccesfull(frv,&(retVal.pkcs11rv), "C_GetSlotList", "test_getprivatekeys" ))
			{
				...
				if(slotIds != NULL)
				{
					...
					if (ReturnedSuccesfull(frv,&(retVal.pkcs11rv), "C_GetSlotList (X2)", "test_getprivatekeys" ))
					{
						if(slot_count == 0)
						{
						...
						for (slotIdx = 0; slotIdx < slot_count; slotIdx++) 
						{
							...
							if(pValue != NULL)
							{
								...
								if (ReturnedSuccesfull(frv,&(retVal.pkcs11rv), "C_OpenSession", "test_getprivatekeys" ))
								{
									frv = (*functions->C_FindObjectsInit)(session_handle, attributes, 1); 
									if (ReturnedSuccesfull(frv,&(retVal.pkcs11rv), "C_FindObjectsInit", "test_getprivatekeys" ))
									{
										frv = (*functions->C_FindObjects)(session_handle, &hKey,1,&ulObjectCount); 
										ReturnedSuccesfull(frv,&(retVal.pkcs11rv), "C_FindObjects", "test_getprivatekeys" ); 
										while (ulObjectCount > 0)
										{
											...
											if (ReturnedSuccesfull(frv,&(retVal.pkcs11rv), "C_GetAttributeValue", "test_getprivatekeys" ))
											{
										...
										ReturnedSuccesfull(frv,&(retVal.pkcs11rv), "C_FindObjectsFinal", "test_getprivatekeys" );
									}
								}
			...
			ReturnedSuccesfull(frv,&(retVal.pkcs11rv), "C_Finalize", "test_getprivatekeys" );
		}
		dlclose(handle);
	}
	else
