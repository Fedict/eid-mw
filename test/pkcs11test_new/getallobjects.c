						...
						for (slotIdx = 0; slotIdx < slot_count; slotIdx++) 
						{
							...
							if(pLabel != NULL)
							{
								...
								if (ReturnedSuccesfull(frv,&(retVal.pkcs11rv), "C_OpenSession", "test_getallobjects" ))
								{
									...
									frv = (*functions->C_FindObjectsInit)(session_handle, NULL, 0); 
									if (ReturnedSuccesfull(frv,&(retVal.pkcs11rv), "C_FindObjectsInit", "test_getallobjects" ))
									{
										...
										while (ulObjectCount > 0)
										{
											...
											if(frv == CKR_ATTRIBUTE_TYPE_INVALID)//object doesn't have one of the specified attributes
											{
												testlog(LVL_INFO, "wrong attribute specified\n");
											}
											else if (ReturnedSuccesfull(frv,&(retVal.pkcs11rv), "C_GetAttributeValue", "test_getallobjects" ))
											{
												testlog(LVL_INFO,"key CKA_LABEL = ");
												testlogbytes(LVL_INFO,(CK_BYTE_PTR)(attr_templ[0].pValue), attr_templ[0].ulValueLen);
												testlog(LVL_INFO,"key CKA_CLASS = %d\n", classvalue);
												if(classvalue == CKO_DATA)
												{
													testlog(LVL_INFO,"key CKA_VALUE = ");
													testlogbytes(LVL_INFO,(CK_BYTE_PTR)(attr_templ[2].pValue), attr_templ[2].ulValueLen);
													testlog(LVL_INFO,"key CKA_VALUE_LEN = %d",attr_templ[3].pValue);
												}
