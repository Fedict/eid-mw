...
int systemGetInfo(System_INFO *info)
{
	...
	wchar_t buff[25];
	if( -1 == _swprintf_s(buff,25,L"%d",osv.dwMajorVersion))
	{
		LOG_ERRORCODE(L"_swprintf_s failed",errno);
		return RETURN_LOG_INTERNAL_ERROR;
	}
	else
	{
	...
	if( -1 == _swprintf_s(buff,25,L"%d",osv.dwMinorVersion))
	{
		LOG_ERRORCODE(L"_swprintf_s failed",errno);
		return RETURN_LOG_INTERNAL_ERROR;
	}
	...
	if( -1 == _swprintf_s(buff,25,L"%d",osv.dwBuildNumber & 0x0000FFFF))
	{
		LOG_ERRORCODE(L"_swprintf_s failed",errno);
		return RETURN_LOG_INTERNAL_ERROR;
	}
...
int systemGetLang(std::wstring *lang)
{
	...
	case LANG_GERMAN:	
		...
		break;
	default :
		if( -1 == _swprintf_s(buff, 6, L"%x-%x", PrimaryLang, SubLang))
		{
	...
	if(bAddSubLang)
	{
		if( -1 == _swprintf_s(buff, 6, L"-%x", SubLang))
		{
			LOG_ERRORCODE(L"Failed to get language",errno);
			return RETURN_LOG_INTERNAL_ERROR;
		}
...
std::wstring GetOSDesc(const OSVERSIONINFOEX &osv) 
{
	...
	switch ( osv.dwPlatformId ) 
	{
		...
		if (osv.dwMajorVersion <= 4) 
		{
			_swprintf_s(tmp, 128, L"version %d.%d %ls (Build %d)",
									osv.dwMajorVersion,
									osv.dwMinorVersion,
		...
		{
			_swprintf_s(tmp, 128, L"%ls (Build %d)",
									GetOSServicePack(osv).c_str(),
									osv.dwBuildNumber & 0x0000FFFF);
			Result.append(L" ");
			Result.append(tmp);
