	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				//FileVersion
				wchar_t Buffer[25];
				if(-1==_swprintf_s(Buffer,25,L"%ld.%ld.%ld.%ld",	infoFixed.dwFileVersionMS / 0x10000,
				...
				//ProductVersion
				if(-1==_swprintf_s(Buffer,25,L"%ld.%ld.%ld.%ld",	infoFixed.dwProductVersionMS / 0x10000,
