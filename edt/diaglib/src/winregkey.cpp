...
WinRegKeyException::WinRegKeyException(LONG errorcode, HKEY rootkey, wstring subkey, wstring label=L"")
...
WinRegKeyException::~WinRegKeyException() throw()
{
}

const LONG	WinRegKeyException::errorcode(void) const {return m_errorcode;}
const wstring	WinRegKeyException::key(void)		 const {return m_key;}
...
:m_rootkey(rootkey),m_subkey(subkey),m_key(0)
{
	...
	if(res!=ERROR_SUCCESS)
		throw WinRegKeyException(res,rootkey,subkey);
}
...
const wstring WinRegKey::operator()(wstring label) const
{
	...
	if(res!=ERROR_SUCCESS)
		throw WinRegKeyException(res,m_rootkey,m_subkey,label);
	return wstring(buf);
}
