...
/* convert variable length binary bit-stream into int-type */
	static unsigned int bin2int(const unsigned char *p_ucDat,
				    unsigned int iLen)
	{
...
/* convert variable length binary bit-stream into unsigned long-type */
	static unsigned long bin2ulong(const unsigned char *p_ucDat,
				       unsigned int iLen)
	{
	...
	static unsigned int p15_bitstring2ul(unsigned char *in,
					     unsigned long l_in)
	{
...
//----------------------------------------------------------------------------------------
	...
	tCommonObjAttr ParseCommonObjectAttributes(ASN1_ITEM * p_xLev0Item)
	{
		...
		//--- get sequence
		if ((p_xLev0Item->l_data < 2)
		    || (asn1_next_item(p_xLev0Item, &xLev1Item) != 0)
		    || (xLev1Item.tag != ASN_SEQUENCE))
			throw CMWEXCEPTION(EIDMW_WRONG_ASN1_FORMAT);
		...
		{
			//get next object
			if ((xLev1Item.l_data < 2)
			    || (asn1_next_item(&xLev1Item, &xLev2Item) != 0))
				throw CMWEXCEPTION(EIDMW_WRONG_ASN1_FORMAT);
			...
			switch (xLev2Item.tag)
			{
				...
				case ASN_OCTET_STRING:	// authority id
					coAttr.ulAuthID =
						bin2ulong(xLev2Item.p_data,
							  xLev2Item.l_data);
					break;

				case ASN_INTEGER:	// user consent
					coAttr.ulUserConsent =
						bin2ulong(xLev2Item.p_data,
							  xLev2Item.l_data);
					break;
		...
		)
	{
		...
		if ((p_xLev0Item->l_data < 2)
		    || (asn1_next_item(p_xLev0Item, &xLev1Item) != 0)
		    || ((xLev1Item.tag) != ASN_OCTET_STRING))
			throw CMWEXCEPTION(EIDMW_WRONG_ASN1_FORMAT);

		//convert to ascii-hex string
		char *p_cBuf =
			bin2AsciiHex(xLev1Item.p_data, xLev1Item.l_data);
		...
		)
	{
		...
		if ((xLev1Item.l_data < 2)
		    || (asn1_next_item(&xLev1Item, &xLev2Item) != 0)
		    || ((xLev2Item.tag) != ASN_OCTET_STRING))
			throw CMWEXCEPTION(EIDMW_WRONG_ASN1_FORMAT);

		//convert to ascii-hex string
		char *p_cBuf =
			bin2AsciiHex(xLev2Item.p_data, xLev2Item.l_data);
...
*/
	tDirInfo PKCS15Parser::ParseDir(const CByteArray & contents)
	{
		...
		if ((asn1_next_item(&xLev0Item, &xLev1Item) != 0)
		    || (xLev1Item.tag != ASNP15_DIRREC))
			throw CMWEXCEPTION(EIDMW_WRONG_ASN1_FORMAT);
		...
		while (xLev1Item.l_data > 0)
		{
			...
			switch (xLev2Item.tag)
			{
				...
				{
					char *p_cBuf =
						bin2AsciiHex(xLev2Item.p_data,
							     xLev2Item.
							     l_data);
					std::string csData(p_cBuf,
							   xLev2Item.l_data *
							   2);
					dir.oAppID = csData;
					delete[]p_cBuf;
				...
				{
					std::string csData((char *) xLev2Item.
							   p_data, 0,
							   xLev2Item.l_data);
					dir.csLabel = csData;
				}
	...
	tTokenInfo PKCS15Parser::ParseTokenInfo(const CByteArray & contents)
	{
		...
		std::string csSerial((char *) xLev2Item.p_data, 0,
				     xLev2Item.l_data);
		...
		{
			std::string csManufactID((char *) xLev2Item.p_data, 0,
						 xLev2Item.l_data);
		...
		{
			std::string csLabel((char *) xLev2Item.p_data, 0,
					    xLev2Item.l_data);
		...
		tInfo.ulFlags =
			p15_bitstring2ul((unsigned char *) xLev2Item.p_data,
					 xLev2Item.l_data);
		...
		while (xLev1Item.l_data > 0)
		{
			...
			switch (xLev2Item.tag)
			{
				case ASN_CONSTRUCTED_CONTEXT_N(1):	//Record Info
				{
					...
					//get sequence
					if ((xLev2Item.l_data < 2)
					    ||
					    (asn1_next_item
					     (&xLev2Item, &xLev3Item) != 0)
					    || (xLev3Item.tag !=
						ASN_SEQUENCE))
						throw CMWEXCEPTION
							(EIDMW_WRONG_ASN1_FORMAT);

					for (int iIndex = 0; iIndex < 7; ++iIndex)	//clear destination
						tInfo.piRecordInfo[iIndex] =
							0;
					// loop to read the integers
					while (xLev3Item.l_data > 0)
					{
						if ((xLev3Item.l_data < 2)
						    ||
						    (asn1_next_item
						     (&xLev3Item,
						      &xLev4Item) != 0))
			...
			//point to next item
			if (xLev2Item.l_raw > xLev1Item.l_data)
				xLev2Item.l_raw = xLev1Item.l_data;	//make sure not to pass the end
	...
	std::vector < tPin >
		PKCS15Parser::ParseAodf(const CByteArray & contents)
	{
#ifdef VERBOSE
		std::cerr <<
			"\n----------- PKCS15Parser::ParseAodf ------------ "
			<< std::endl;
#endif
		...
		while (xLev0Item.l_data > 0)
		{
			...
			if ((xLev0Item.l_data < 2)
			    || (asn1_next_item(&xLev0Item, &xLev1Item) != 0))
				throw CMWEXCEPTION(EIDMW_WRONG_ASN1_FORMAT);
			...
			if (xLev1Item.tag == ASN_SEQUENCE)	//level.1 sequence
			{
				...
				if ((xLev1Item.l_data < 2)
				    || (asn1_next_item(&xLev1Item, &xLev2Item)
					!= 0)
				    || (xLev2Item.tag != ASN_SEQUENCE))
					throw CMWEXCEPTION
						(EIDMW_WRONG_ASN1_FORMAT);

				// Authority ID 
				if ((xLev2Item.l_data < 2)
				    || (asn1_next_item(&xLev2Item, &xLev3Item)
					!= 0)
				    || (xLev3Item.tag != ASN_OCTET_STRING))
					throw CMWEXCEPTION
						(EIDMW_WRONG_ASN1_FORMAT);

				pin.ulID =
					bin2ulong(xLev3Item.p_data,
						  xLev3Item.l_data);
					...
					{	/*
						...
						if ((xLev3Item.l_data < 2)
							...
							throw CMWEXCEPTION
								(EIDMW_WRONG_ASN1_FORMAT);
						pin.ulPinFlags =
							p15_bitstring2ul((unsigned char *) xLev4Item.p_data, xLev4Item.l_data);
			...
			if (xLev2Item.tag == ASN_SEQUENCE)
			{
				...
				switch (iItemTag)
				{
					...
					case ASN_CONSTRUCTED_CONTEXT_N(2):
						result.csTrustedPukdfPath =
							csPath;
						break;
					case ASN_CONSTRUCTED_CONTEXT_N(3):
						result.csSecretPukdfPath =
							csPath;
						break;
					...
					case ASN_CONSTRUCTED_CONTEXT_N(5):
						result.csTrustedCdfPath =
							csPath;
						break;
					case ASN_CONSTRUCTED_CONTEXT_N(6):
						result.csUsefulCdfPath =
							csPath;
						break;
	...
	std::vector < tPrivKey >
		PKCS15Parser::ParsePrkdf(const CByteArray & contents)
	{
		...
		while (xLev0Item.l_data > 0)
		{
			...
			if (xLev1Item.tag == ASN_SEQUENCE)
			{
				...
				if ((xLev2Item.l_data < 2)
				    || (asn1_next_item(&xLev2Item, &xLev3Item)
					!= 0)
				    || (xLev3Item.tag != ASN_BIT_STRING))
					throw CMWEXCEPTION
						(EIDMW_WRONG_ASN1_FORMAT);
				xKey.ulKeyUsageFlags =
					p15_bitstring2ul((unsigned char *)
							 xLev3Item.p_data,
							 xLev3Item.l_data);
				    ...
				    ASN_CONSTRUCTED_CONTEXT_N(1))
				{
					...
					if ((xLev3Item.l_data < 2)
					    ||
					    (asn1_next_item
					     (&xLev3Item, &xLev4Item) != 0)
					    || (xLev4Item.tag != ASN_INTEGER))
						throw CMWEXCEPTION
							(EIDMW_WRONG_ASN1_FORMAT);
	...
	std::vector < tCert >
		PKCS15Parser::ParseCdf(const CByteArray & contents)
	{
