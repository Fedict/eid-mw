...
namespace eIDMW
{
...

		{
			...
			try
			{
				...
				bool bIsBeidCard = oData.Size() == 2
					...
					&& oData.GetByte(1) == 0x00;
				if (bIsBeidCard)
					poCard = new CCard(hCard, poContext,
						poPinpad, oData, bNeedToSelectApplet ? ALW_SELECT_APPLET : TRY_SELECT_APPLET);
	...
	void CCard::SetSecurityEnv(const tPrivKey & key, unsigned long algo,
	...
	unsigned char CCard::PinUsage2Pinpad(const tPin & Pin,
		const tPrivKey * pKey)
	{
		...
		const tPrivKey * pKey)
	{
		...
		return PinCmd_2(operation, beidPin, csPin1, csPin2,
			ulRemaining, pKey);
	}
	...
	bad_pin:
		...
		CByteArray CCard::VerifyAndSign(const tPrivKey & key,
			const tPin & Pin,
			unsigned long algo,
			const CByteArray & oData)
		{
		...
		CByteArray CCard::ReadBinary(unsigned long ulOffset,
			unsigned long ulLen)
		{
			// Read Binary
		...
		CByteArray CCard::UpdateBinary(unsigned long ulOffset,
			const CByteArray & oData)
		{
			// Update Binary
