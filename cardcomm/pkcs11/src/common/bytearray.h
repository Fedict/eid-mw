...
public:
		CByteArray(unsigned long ulCapacity = 0);
		     CByteArray(const unsigned char *pucData,
				unsigned long ulSize,
				unsigned long ulCapacity = 0);
		     CByteArray(const CByteArray & oByteArray);
		      CByteArray(const std::string & csData, bool bIsHex =
				 false);
		     ~CByteArray();
	...
	/** Create a new CByteArray with part of this */
		CByteArray GetBytes(unsigned long ulOffset,
		...
		void Append(const CByteArray & oByteArray);
		      CByteArray & operator +=(const CByteArray & oByteArray);
		void Append(const unsigned char *pucData,
			    unsigned long ulSize);
	...
	*/
		void Replace(unsigned char ucByteSrc,
    ...
    /** Returns a hex string, either on 1 line and truncated if needed
      ...
      * A value of 0xFFFFFFFF for ulLen means: till the end of the bytearray. */
		      std::wstring ToWString(bool bAddSpace =
					     true, bool bOneLine =
					     true, unsigned long ulOffset =
					     0, unsigned long ulLen =
					     0xFFFFFFFF) const;
		      std::string ToString(bool bAddSpace =
					   true, bool bOneLine =
					   true, unsigned long ulOffset =
					   0, unsigned long ulLen =
					   0xFFFFFFFF) const;
