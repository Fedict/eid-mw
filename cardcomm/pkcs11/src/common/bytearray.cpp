...
namespace eIDMW
{
	...
	CByteArray::CByteArray(unsigned long ulCapacity):m_pucData(NULL),
		m_ulSize(0), m_ulCapacity(ulCapacity), m_bMallocError(false)
	{
	}

//copy mem into object
	CByteArray::CByteArray(const unsigned char *pucData,
			       unsigned long ulSize, unsigned long ulCapacity)
	{
	...
	CByteArray & CByteArray::operator =(const CByteArray & oByteArray)
	{
		if (&oByteArray != this)	//only action needed if both are not the same object
		{
			...
			else if (m_ulCapacity >= oByteArray.Size())
			{
				...
				memcpy(m_pucData, oByteArray.GetBytes(),
				       m_ulSize);
	...
	CByteArray::CByteArray(const std::string & csData, bool bIsHex)
	{
		if (!bIsHex)
		{
			const unsigned char *data =
				reinterpret_cast <
				const unsigned char *>(csData.c_str());
			MakeArray(data,
				  static_cast <
				  unsigned int >(csData.length()));
		} else
		{
...
// const unsigned char *CByteArray::GetBytes() const
	...
	CByteArray CByteArray::GetBytes(unsigned long ulOffset,
...
//Other formalism for the Append(unsigned char ucByte)
	...
	void CByteArray::AppendLong(unsigned long ulLong)
	{
		...
		{
			tucLong[i] =
				static_cast < unsigned char >(ulLong % 256);
			ulLong /= 256;
		}
...
//Other formalism for the Append(const CByteArray & oByteArray)
	...
	void CByteArray::Append(const unsigned char *pucData,
				unsigned long ulSize)
	{
		...
		{
			if (m_ulSize + ulSize > m_ulCapacity
			    || m_pucData == NULL)
			{
				m_ulCapacity =
					m_ulSize + ulSize +
				...
				m_pucData =
					static_cast <
					unsigned char
					*>(realloc(m_pucData, m_ulCapacity));
				if (m_pucData == NULL)
				{
					m_bMallocError = true;
					throw CMWEXCEPTION(EIDMW_ERR_MEMORY);
...
// void CByteArray::Append()
	...
	{
		Append((const unsigned char *) (scData.c_str()),
...
// void CByteArray::SecureClearContents() set content value to zero's before freeing
	...
	bool CByteArray::Equals(const CByteArray & oByteArray) const
	{
		...
		if    (m_pucData == NULL)
			...
			memcmp(m_pucData, oByteArray.GetBytes(),
			       m_ulSize) == 0;
	}
	...
	static inline void AddHexChar(char *csBuf, unsigned long &ulOffset,
				      bool bAddSpace, unsigned char uc)
	{
	...
	std::wstring CByteArray::ToWString(bool bAddSpace, bool bOneLine,
					   ...
					   unsigned long ulLen) const
	{
		return utilStringWiden(ToString
				       (bAddSpace, bOneLine, ulOffset,
					ulLen));
	}
					  ...
					  unsigned long ulLen) const
	{
		...
		if   (bOneLine)
		{
			...
			for  (i = 0; i < ulEnd; i++)
				     AddHexChar(csBuf, ulBufOffset, bAddSpace,
						pTmp[i]);
			if   (ulLen > 16)
			{
				csBuf[ulBufOffset++] = '.';
		...
		} else
		{
			...
			{
				unsigned long ulEnd =
					ulLen - i > 16 ? 16 : ulLen - i;
				ulBufOffset = 0;
				csBuf[ulBufOffset++] = '\t';
				for (unsigned long j = 0; j < ulEnd; j++)
					AddHexChar(csBuf, ulBufOffset,
						   bAddSpace, pTmp[j]);
				pTmp += ulEnd;
				csBuf[ulBufOffset++] = '\n';
				csBuf[ulBufOffset] = '\0';
...
//?? capacity is not in steps of 10, but takes new data-len
				   ...
				   unsigned long ulCapacity)
	{
		...
		if (m_ulCapacity == 0)
			m_ulCapacity = EXTRA_INCREMENT_LEN;
		m_pucData =
			static_cast < unsigned char *>(malloc(m_ulCapacity));
		if (m_pucData == NULL)
		{
			m_ulSize = 0;
			m_bMallocError = true;
