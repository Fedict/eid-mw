...
/** Unstream a multi-TLV-block into m_oMapTLV structure.
 ...
 returns: false = invalid data
          true  = OK all done
*/
	bool CTLVBuffer::ParseFileTLV(const unsigned char *pucData,
				      unsigned long ulLen)
	{
		bool bRet = false;
...
//            int iNrBytes = ulLen - ulIndex - 1;     //calculate remaining bytes
				...
				//--- add the decoded TLV 
				m_oMapTLV[ucTag] =
					new CTLV(ucTag, pucData + ulIndex,
						 ulFieldLen);
				ulIndex += ulFieldLen;
			}
		}
 ...
 */
	int CTLVBuffer::ParseTLV(const unsigned char *pucData,
				 unsigned long ulLen)
	{
		int iRet = 0;
					      ...
					      unsigned long *pulLen)
	{
		...
		if ((NULL != (pTagData = GetTagData(ucTag)))
		    && (pData != NULL) && (pulLen != NULL))
		{
			char *pszTemp =
				Hexify(pTagData->GetData(),
				       pTagData->GetLength());
			if (*pulLen >
			    (ulLength = (unsigned long) strlen(pszTemp)))
			{
	...
	void CTLVBuffer::FillLongData(unsigned char ucTag, long *piData)
	{
		...
		if (NULL != (pTagData = GetTagData(ucTag)))
		{
			...
			memcpy(pszTemp, pTagData->GetData(),
			       pTagData->GetLength());
			*piData = atol(pszTemp);
			delete[]pszTemp;
		}
	...
	void CTLVBuffer::FillBinaryData(unsigned char ucTag, unsigned char *pData, unsigned long *pulLen
							    /**< in/out: In=max.length, Out=current length */
		)
	{
		CTLV *pTagData = NULL;
	...
	bool CTLVBuffer::FillBinaryDataCheck(unsigned char ucTag,
					     unsigned char *pData,
					     unsigned long *pulLen)
	{
		bool bRet = false;
 ...
 **********************************************************************************************************/
		...
		)
	{
		...
		*piBufLen = iStreamLen;
		...
		while (iStreamLen--)
		{
			pucBufDest[iStreamLen] =
				((unsigned char) ulLenValLoc & 0x7F) +
				ucExtraByte;
			ucExtraByte = 0x80;
			ulLenValLoc >>= 7;
		}
 ...
 **********************************************************************************************************/
	...
	void CTLVBuffer::SetTagData(unsigned char ucTag,
	...
	unsigned long CTLVBuffer::Extract(unsigned char *pucData,
					  unsigned long ulLen)
	{
		...
		for (it = m_oMapTLV.begin(); it != m_oMapTLV.end(); ++it)
		{
			...
			if (TlvEncodeLen
			    (it->second->GetLength(), ucLenBuffer, &iBufLen))
			{
				oTempArray.Append(it->second->GetTag());
				oTempArray.Append(ucLenBuffer, iBufLen);
				oTempArray.Append(it->second->GetData(),
						  it->second->GetLength());
			}
		}
