...
static const string BEID_PKCS11DLL = "BeidPkcs11.dll";


/// Windows message for the slot event notification
static const DWORD CM_P11SLOTEVENT = WM_APP + 0x801;
...
/// slot event monitor thread

struct CP11ThreadParams {
    CK_FUNCTION_LIST_PTR FP11;
    CPKCS11* P11;
};

class CP11Thread : public CThread {
private:
    CK_FUNCTION_LIST_PTR FP11;
    CPKCS11* P11;

public:
    unsigned long Execute(void* parameters);
};
...
private:
    ...
    /// the slot monitor thread
    CP11Thread FSlotMonitor;
    /// The slot event notification window handle (see SlotEventWndProc)
    HWND FWindowHandle;
    /// Return the number of slots
    unsigned int GetSlotCount();
    /// Returns a SlotInfo
    CP11SlotInfo& GetSlot(UINT Index);
    /// The Windows Msg callback that runs in the main thread context
    void SlotEventWndproc(MSG message)/* MESSAGE CM_P11SLOTEVENT */;
...
public:
    ...
    static string P11StringToString(CK_UTF8CHAR_PTR src, size_t size);
...
protected:
public:
    CP11SessionInfo(CP11TokenInfo& Token, CK_FUNCTION_LIST_PTR P11FunctionList, CK_SLOT_ID SlotId, const string& PinCode = "");
    virtual ~CP11SessionInfo(void);
    /// Compute a message digest on a byte string
    Byte_Array Digest(CK_MECHANISM_TYPE Algorithm, const string& DataMessage, int *length);
    /// Login to the token
    bool Login(CK_USER_TYPE Mode, const string& PinCode);
    /// Logout from the token
    bool Logout();
    /// Get a pkcs11 digital signature
    Byte_Array Sign(CP11Object KeyObject, CK_MECHANISM_TYPE Algorithm, const string& DataMessage, int *length);
