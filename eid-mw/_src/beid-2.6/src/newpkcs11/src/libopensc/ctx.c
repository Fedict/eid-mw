...
/* Default Belpic settings */
#define OPENSC_CONFIG_STRING "app default {\n framework pkcs15 {\n use_caching = true;\n }\n }\n app opensc-pkcs11 {\n pkcs11 {\n num_slots = 2;\n hide_empty_tokens = true;\n lock_login = false;\n }\n }\n"

/* Belpic: the older belpic cards have a bug: fast readers send
 * a Get Response APDU too fast, so the card can't handle it.
 * Our solution is a configurable delay (in msecs) before
 * each Get Response APDU */
#define DEF_GET_RESP_DELAY	50
...
static const struct _sc_driver_entry internal_card_drivers[] = {
	{ "belpic", (void *) sc_get_belpic_driver, NULL }, /* Belpic */
#ifdef HAVE_ALL_CARDS /* Belpic */
...
{
	ctx->debug = 0;
	ctx->disable_errors = 1; /* Belpic */
	ctx->allow_sso = 1; /* Belpic */
			   ...
			   struct _sc_ctx_options *opts)
{
	...
	ctx->disable_errors = scconf_get_bool(block, "disable_errors", ctx->disable_errors); /* Belpic */
#ifdef __NO_SSO__
	ctx->allow_sso = 0;
#else
	ctx->allow_sso = scconf_get_bool(block, "allow_sso", ctx->allow_sso); /* Belpic */
#endif
	ctx->get_response_delay = scconf_get_int(block, "get_response_delay", DEF_GET_RESP_DELAY); /* Belpic */
	val = scconf_get_str(block, "debug_file", NULL);
	if (val) {
#ifdef BELPIC_HAVE_DEBUG /* Belpic: no debugging in the release */
		if (ctx->debug_file && ctx->debug_file != stdout)
			fclose(ctx->debug_file);
	...
	if (val) {
		if (ctx->error_file && ctx->error_file != stderr)
			fclose(ctx->error_file);
		if (strcmp(val, "stderr") != 0 && !ctx->disable_errors) /* Belpic */
