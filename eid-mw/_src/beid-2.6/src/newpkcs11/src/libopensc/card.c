...
int sc_transmit_apdu(struct sc_card *card, struct sc_apdu *apdu)
{
	...
	if (apdu->sw1 == 0x6C && apdu->resplen == 0) {
		...
		/* Belpic: fix for applet V1.0 cards, see ctx.c for more info */
		if (belpic_get_globalos_version(card) == 0x0001) {
#ifdef _WIN32
			Sleep(card->ctx->get_response_delay);
#else
			usleep(1000 * card->ctx->get_response_delay);
#endif
		}
