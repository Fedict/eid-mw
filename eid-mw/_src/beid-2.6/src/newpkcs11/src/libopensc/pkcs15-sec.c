				...
				u8 *out, size_t outlen)
{
	...
	if (flags & SC_ALGORITHM_RSA_PSS_SHA1) {
			senv.algorithm_flags |= SC_ALGORITHM_RSA_PSS_SHA1;
	} else if (flags & SC_ALGORITHM_RSA_HASH_SHA1) {
		if (inlen != 20)
	...
	if (flags & SC_ALGORITHM_RSA_PAD_PKCS1) {
		...
		if (!(alg_info->flags & SC_ALGORITHM_RSA_PSS_SHA1)) {
			sc_error(ctx, "Card requires RSA padding\n");
			return SC_ERROR_NOT_SUPPORTED;
		}
		senv.algorithm_flags |= SC_ALGORITHM_RSA_RAW;
	...
	/* Fix for current beta V2 card that don't support naked pkcs1:
	 * if we get a request for a naked pkcs1 signature, try to strip
	 * off the SHA-1 or MD5 header.
	 *
	 * Should preferably be removed in when the V2 cards are fixed.
	 */
	if (senv.algorithm_flags & SC_ALGORITHM_RSA_PAD_PKCS1) {
		if ((inlen == 35)) { // && !memcmp(in, "\x30\x21\x30\x09\x06\x05\x2B\x0E\x03\x02\x1A\x05\x00\x04\x14", 15)) {
			in += 15;
			inlen = 20;
			senv.algorithm_flags = SC_ALGORITHM_RSA_HASH_SHA1;
		}
		if ((inlen == 34)) { // && !memcmp(in, "\x30\x20\x30\x0C\x06\x08\x2A\x86\x48\x86\xF7\x0D\x02\x05\x05\x00\x04\x10", 18)) {
			inlen = 16;
			in += 18;
			senv.algorithm_flags = SC_ALGORITHM_RSA_HASH_MD5;
		}
