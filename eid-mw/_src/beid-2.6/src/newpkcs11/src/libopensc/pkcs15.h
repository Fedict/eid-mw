...
typedef struct sc_pkcs15_card {
	...
	/* From the belpic EF(TokenInfo) */
	int graphicalversion;
	int persoversion;
	int persointerfaceversion;

	unsigned long flags;

	/* Belpic: last 16 bytes of the ID signature file (3F00/DF01/4032) */
	unsigned char cache_key[FILE_PART_LEN];
	int cache_key_read;
...
/* Idem but with data buffer already allocated */
